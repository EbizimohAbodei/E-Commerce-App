  ...
  rest.get("https://api.escuelajs.co/api/v1/products", (req, res, ctx) => {
    ...
    if (price_min && price_max) {
      response = products.filter(
        (product) =>
          product.price >= Number(filter.price_min) &&
          product.price <= Number(filter.price_max)
      );
    } else if (Number(categoriesSearch) > 0) {
      response = products.filter((product) => product.category.id === 1);
    } else if (titleSearch) {
      response = products.filter((product) => product.title === "Product 3");
    } else {
      response = products;
    }
  ...
  rest.post(
    ...
    async (req, res, ctx) => {
      ...
      if (!Array.isArray(newProduct.images)) {
        ...
        newProduct.images.some((item: any) => typeof item !== "string")
      ) {
      ...
      return res(ctx.status(201), ctx.json(products));
    }
  ),
